...
- **Storage hierarchy:** `Freezer → Rack → Box → Specimen (PositionRow/PositionCol)`. A unique index on `(BoxID, PositionRow, PositionCol)` enforces conflict detection at the DB level.
